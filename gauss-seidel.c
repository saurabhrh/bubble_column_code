...
int main()
{
    int i,j,nx=NX-1,ny=NY-1,inner,outer,Kx=nx*NY,Ky=NX*ny,Kp=NX*NY,m,time,numsaves=TIMESTEPS/SAVING_INTERVAL,count=0,z_e,z_w,z_n,z_s,c_count=0;
    FILE *resuc;
    FILE *resvc;
    FILE *resud;
    FILE *resvd;
    ...
    double DX2=DX*DX,DY2=DY*DY,ar=DY/DX,ra=1/ar,U_x,V_y,U_y,V_x,top,left,right,bottom,AR=ar/SIGMAK,RA=ra/SIGMAK,R=ra/SIGMAEPS,r=ar/SIGMAEPS,alphacn_centre,alphadn_centre,xp=DX/2,up,vp,tau_y,mu_w,mu_s,mu_e,mu_n,v_plus,x_plus,kp,fE,fW,fN,fS,mut_e,mut_n,mut_s,mut_w,nor,residual_y_c,residual_y_d,residual_P,uE,uW,vN,vS,residual_eps,residual_k,residual_x_c,residual_x_d,residual_alpha,alc_e,alc_w,alc_n,alc_s,E,Eo,Re_b,C_D,alpe_c,alpn_c,alps_c,alpw_c,alpe_d,alpw_d,alpn_d,alps_d,ald_e,ald_w,ald_n,ald_s,U_mc,U_md,U_diffmag,V_md,V_mc,mdot_c,mdot_d,a_if,R_1,R2eps,R2p,R2k,R2a,alphad_centre,alphac_centre,Edash,C_D_Churn,C_D_Distorted,C_D_Newton,U_diffmagx,U_diffmagy,kappa = KAPPA*xp,DT=1/dt,cpu_time,R_scale,scale_res_p;
    clock_t start,end;

    double ** u_c = calloc((NX+1),sizeof(double *));		// u-velocity continuous phase
    double ** u_d = calloc((NX+1),sizeof(double *));		// u-velocity dispersed phase
    double ** B_kx = calloc((NX+1),sizeof(double *));		// Interface exchange coefficient from steady drag for x-momentum
    double ** DIV_xc = calloc((NX+1),sizeof(double *));		// Divergence of velocity in x-momentum continuous phase
    double ** DIV_xd = calloc((NX+1),sizeof(double *));		// Divergence of velocity in x-momentum dispersed phase
    double ** un_d = calloc((NX+1),sizeof(double *));		// u-velocity continuous phase at previous timestep
    double ** un_c = calloc((NX+1),sizeof(double *));		// u-velocity dispersed phase at previous timestep
    double ** F_added_x = calloc((NX+1),sizeof(double *));	// Added mass force in x-direction
    double ** mat_der_x = calloc((NX+1),sizeof(double *));	// Material derivative in x-momentum eqn
    for (i=0;i<NX+1;i++)
    {
    ...
    double ** v_c = calloc(NX,sizeof(double *));		// v-velocity for continuous phase
    double ** v_d = calloc(NX,sizeof(double *));		// v-velocity for dispersed phase
    double ** B_ky = calloc(NX,sizeof(double *));		// Interface exchange coefficient from steady drag for y-direction
    double ** DIV_yc = calloc(NX,sizeof(double *));		// Divergence of velocity in y-momentum continuous phase
    double ** DIV_yd = calloc(NX,sizeof(double *));		// Divergence of velocity in y-momentum dispersed phase
    double ** vn_d = calloc(NX,sizeof(double *));		// v-velocity continuous phase at previous timestep
    double ** vn_c = calloc(NX,sizeof(double *));		// v-velocity dispersed phase at previous timestep
    double ** F_added_y = calloc(NX,sizeof(double *));	// Added mass force in y-direction
    double ** mat_der_y = calloc(NX,sizeof(double *));	// Material derivative in y-momentum eqn
    for (i=0;i<NX;i++)
    {
    ...
    double ** alpha_d = calloc(NX,sizeof(double *));	// Volume fraction dispersed phase
    double ** alpha_c = calloc(NX,sizeof(double *));	// Volume fraction continuous phase
    double ** P = calloc(NX,sizeof(double *));			// Pressure
    double ** mut_C = calloc(NX,sizeof(double *));		// Continuous phase turbulent viscosity
    double ** k = calloc(NX,sizeof(double *));			// Turbulent kinetic energy
    double ** epsilon = calloc(NX,sizeof(double *));	// Turbulence dissipation rate
    double ** mu_total_C = calloc(NX,sizeof(double *));	// Effective viscosity continuous phase
    double ** mu_total_D = calloc(NX,sizeof(double *));	// Effective viscosity dispersed phase
    double ** uslip_mag = calloc(NX,sizeof(double *));	// Slip Velocity
    double ** DIV = calloc(NX,sizeof(double *));		// Divergence of velocity
    double ** source = calloc(NX,sizeof(double *));		// Source term in turbulence eqn
    double ** production = calloc(NX,sizeof(double *));	// Production term in k-eqn
    double ** kn = calloc(NX,sizeof(double *));			// k of previous timestep
    double ** epsn = calloc(NX,sizeof(double *));		// epsilon of previous timestep
    double ** alphan_d = calloc(NX,sizeof(double *));	// Volume fraction dispersed phase of previous timestep
    double ** alphan_c = calloc(NX,sizeof(double *));	// Volume fraction continuous phase of previous timestep
    double ** residual = calloc(NX,sizeof(double *));
    for (i=0;i<NX;i++)
    {
    ...
    double * k_inlet = calloc(NX,sizeof(double));		// Inlet value of k
    double * v_inlet = calloc(NX,sizeof(double));		// Inlet value of v-velocity dispersed phase 
    double * eps_inlet = calloc(NX,sizeof(double));		// Inlet value of epsilon
    double * mut_c_inlet = calloc(NX,sizeof(double));	// Inlet value of turbulent viscosity of continuous phase
    double * mu_total_inletc = calloc(NX,sizeof(double));	// Effective viscosity of continuous phase at inlet
    double * mu_total_inletd = calloc(NX,sizeof(double));	// Effective viscosity of dispersed phase at inlet
    double * muxc_inlet = calloc(NX,sizeof(double));		// Interpolated Effective viscosity of continuous phase at inlet in x-momentum
    double * muxd_inlet = calloc(NX,sizeof(double));		// Interpolated Effective viscosity of dispersed phase at inlet in x-momentum
    double * alphad_inlet = calloc(NX,sizeof(double));		// Inlet value of dispersed phase volume fracton
    double * alphac_inlet = calloc(NX,sizeof(double));		// Inlet value of continuous phase volume fracton
    double * u_cinlet = calloc(NX+1,sizeof(double));		// Inlet value of u-velocity continuous phase
    double * u_dinlet = calloc(NX+1,sizeof(double));		// Inlet value of u-velocity dispersed phase
    double * Uc = calloc(Kx,sizeof(double));			// u-velocity continuous phase in 1D vector form
    double * Vc = calloc(Ky,sizeof(double));			// v-velocity continuous phase in 1D vector form	
    double * Ud = calloc(Kx,sizeof(double));			// u-velocity dispersed phase in 1D vector form
    double * Vd = calloc(Ky,sizeof(double));			// v-velocity dispersed phase in 1D vector form
    double * p = calloc(Kp,sizeof(double));				// pressure in 1D vector form
    double * K = calloc(Kp,sizeof(double));				// k in 1D vector form
    double * Eps = calloc(Kp,sizeof(double));			// epsilon in 1D vector form
    double * a = calloc(Kp,sizeof(double));				// volume fraction of dispersed phase in 1D vector form
    ...
    double * s_xd = calloc(Kx,sizeof(double));
    double * rtilde_xd = calloc(Kx,sizeof(double));
    double * res_xd_initial = calloc(Kx,sizeof(double));
    double * resxdold = calloc(Kx,sizeof(double));
    double * G_xd = calloc(Kx,sizeof(double));
    double * v_xd = calloc(Kx,sizeof(double));
    double * t_xd = calloc(Kx,sizeof(double));
    double * vec_Yxd_t = calloc(Kx,sizeof(double));
    double * vec_Yxd_s = calloc(Kx,sizeof(double));
    double * vec_deltaxd_t = calloc(Kx,sizeof(double));
    double * vec_deltaxd_s = calloc(Kx,sizeof(double));
    ...
    double * s_xc = calloc(Kx,sizeof(double));
    double * rtilde_xc = calloc(Kx,sizeof(double));
    double * res_xc_initial = calloc(Kx,sizeof(double));
    double * resxcold = calloc(Kx,sizeof(double));
    double * G_xc = calloc(Kx,sizeof(double));
    double * v_xc = calloc(Kx,sizeof(double));
    double * t_xc = calloc(Kx,sizeof(double));
    double * vec_Yxc_t = calloc(Kx,sizeof(double));
    double * vec_Yxc_s = calloc(Kx,sizeof(double));
    double * vec_deltaxc_t = calloc(Kx,sizeof(double));
    double * vec_deltaxc_s = calloc(Kx,sizeof(double));
    ...
    double * s_yc = calloc(Ky,sizeof(double));
    double * rtilde_yc = calloc(Ky,sizeof(double));
    double * res_yc_initial = calloc(Ky,sizeof(double));
    double * resycold = calloc(Ky,sizeof(double));
    double * G_yc = calloc(Ky,sizeof(double));
    double * v_yc = calloc(Ky,sizeof(double));
    double * t_yc = calloc(Ky,sizeof(double));
    double * vec_Yyc_t = calloc(Ky,sizeof(double));
    double * vec_Yyc_s = calloc(Ky,sizeof(double));
    double * vec_deltayc_t = calloc(Ky,sizeof(double));
    double * vec_deltayc_s = calloc(Ky,sizeof(double));
    ...
    double * s_yd = calloc(Ky,sizeof(double));
    double * rtilde_yd = calloc(Ky,sizeof(double));
    double * res_yd_initial = calloc(Ky,sizeof(double));
    double * resydold = calloc(Ky,sizeof(double));
    double * G_yd = calloc(Ky,sizeof(double));
    double * v_yd = calloc(Ky,sizeof(double));
    double * t_yd = calloc(Ky,sizeof(double));
    double * vec_Yyd_t = calloc(Ky,sizeof(double));
    double * vec_Yyd_s = calloc(Ky,sizeof(double));
    double * vec_deltayd_t = calloc(Ky,sizeof(double));
    double * vec_deltayd_s = calloc(Ky,sizeof(double));
    ...
    double * s_p = calloc(Kp,sizeof(double));
    double * rtilde_p = calloc(Kp,sizeof(double));
    double * res_p_initial = calloc(Kp,sizeof(double));
    double * respold = calloc(Kp,sizeof(double));
    double * G_p = calloc(Kp,sizeof(double));
    double * v_p = calloc(Kp,sizeof(double));
    double * t_p = calloc(Kp,sizeof(double));
    double * vec_Yp_t = calloc(Kp,sizeof(double));
    double * vec_Yp_s = calloc(Kp,sizeof(double));
    double * vec_deltap_t = calloc(Kp,sizeof(double));
    double * vec_deltap_s = calloc(Kp,sizeof(double));
    ...
    double * s_k = calloc(Kp,sizeof(double));
    double * rtilde_k = calloc(Kp,sizeof(double));
    double * res_k_initial = calloc(Kp,sizeof(double));
    double * reskold = calloc(Kp,sizeof(double));
    double * G_k = calloc(Kp,sizeof(double));
    double * v_k = calloc(Kp,sizeof(double));
    double * t_k = calloc(Kp,sizeof(double));
    double * vec_Yk_t = calloc(Kp,sizeof(double));
    double * vec_Yk_s = calloc(Kp,sizeof(double));
    double * vec_deltak_t = calloc(Kp,sizeof(double));
    double * vec_deltak_s = calloc(Kp,sizeof(double));
    ...
    double * s_eps = calloc(Kp,sizeof(double));
    double * rtilde_eps = calloc(Kp,sizeof(double));
    double * res_eps_initial = calloc(Kp,sizeof(double));
    double * resepsold = calloc(Kp,sizeof(double));
    double * G_eps = calloc(Kp,sizeof(double));
    double * v_eps = calloc(Kp,sizeof(double));
    double * t_eps = calloc(Kp,sizeof(double));
    double * vec_Yeps_t = calloc(Kp,sizeof(double));
    double * vec_Yeps_s = calloc(Kp,sizeof(double));
    double * vec_deltaeps_t = calloc(Kp,sizeof(double));
    double * vec_deltaeps_s = calloc(Kp,sizeof(double));
    ...
    double * s_a = calloc(Kp,sizeof(double));
    double * rtilde_a = calloc(Kp,sizeof(double));
    double * res_a_initial = calloc(Kp,sizeof(double));
    double * resaold = calloc(Kp,sizeof(double));
    double * G_a = calloc(Kp,sizeof(double));
    double * v_a = calloc(Kp,sizeof(double));
    double * t_a = calloc(Kp,sizeof(double));
    double * vec_Ya_t = calloc(Kp,sizeof(double));
    double * vec_Ya_s = calloc(Kp,sizeof(double));
    double * vec_deltaa_t = calloc(Kp,sizeof(double));
    double * vec_deltaa_s = calloc(Kp,sizeof(double));
...
for (time=1;time<=TIMESTEPS;time++)
{
    ...
    {
        double ** uc_ap = calloc(NX,sizeof(double *));		// Central coefficient of x-momentum continuous phase
        double ** uc_an = calloc(NX,sizeof(double *));		// North coefficient of x-momentum continuous phase
        double ** uc_as = calloc(NX,sizeof(double *));		// South coefficient of x-momentum continuous phase
        double ** uc_ae = calloc(NX,sizeof(double *));		// East coefficient of x-momentum continuous phase
        double ** uc_aw = calloc(NX,sizeof(double *));		// West coefficient of x-momentum continuous phase
        for (i=0;i<NX;i++)
        {
        ...
        // Calculation of Material Derivative for x-momentum eqn.
        ...
        // Calculation of Interfacial forces for x-momentum equation
        ...
        for (i=1;i<NX;i++)
        {
            for (j=0;j<NY;j++)
            {
                ...
                U_mc = u_c[i][j];		// Cell-center u-velocity
                V_mc = (v_c[i][j] + v_c[i][j+1] + v_c[i-1][j] + v_c[i-1][j+1])/4;	// Cell-center v-velocity

                U_md = u_d[i][j];
                V_md = (v_d[i][j] + v_d[i][j+1] + v_d[i-1][j] + v_d[i-1][j+1])/4;

                U_diffmagx = sqrt( pow((U_md - U_mc),2) + pow((V_md - V_mc),2) );	// Magnitude of slip-velocity

                Re_b = RHO_C*U_diffmagx*DP/mu;		// Bubble reynolds number

                // Calculation of Drag Coefficient according Schiller-Naumann law
                ...
                a_if = 6*alphad_centre/DP;		// Interfacial area

                B_kx[i][j] = alphac_centre*C_D*a_if*RHO_C*U_diffmagx*DX2/8;	// Interfacial Momentum Exchange coefficient
                ...
                F_added_x[i][j] = 0.5*RHO_C*alphad_centre*alphac_centre*( ((u_c[i][j] - u_d[i][j]) - (un_c[i][j] - un_d[i][j]))*DT + mat_der_x[i][j])*DX2;// + u_c[i][j]*( u_c[i+1][j] - u_c[i-1][j] )/(2*DX) +  V_mc*( u_c[i][j+1] - u_c[i][j-1] )/(2*DX) - u_d[i][j]*( u_d[i+1][j] - u_d[i-1][j] )/(2*DX) +  V_md*( u_d[i][j+1] - u_d[i][j-1] )/(2*DX) )*DX2;
        ...
        for (i=1;i<NX;i++)
        {
            for (j=1;j<NY-1;j++)
            {
                ...
                // Product of effective viscosity and volume fraction at the faces
                ...
                // Effective mass flux across the faces
                ...
                // Switching function values for TVD scheme
        ...
        // Interfacial Force calculaton for y-mom eqn.
        ...
        for (i=0;i<NX;i++)
        {
            for (j=1;j<NY;j++)
            {
                ...
                B_ky[i][j] = alphac_centre*C_D*a_if*RHO_C*U_diffmagy*DX2/8;
                ...
                F_added_y[i][j] = 0.5*RHO_C*alphad_centre*alphac_centre*( ( (v_c[i][j] - v_d[i][j]) - (vn_c[i][j] - vn_d[i][j]) )*DT + mat_der_y[i][j] )*DX2;// + U_mc*( v_c[i+1][j] - v_c[i-1][j] )/(2*DY) + v_c[i][j]*( v_c[i][j+1] - v_c[i][j-1] )/(2*DY) - U_md*( v_d[i+1][j] - v_d[i-1][j] )/(2*DY) + v_d[i][j]*( v_d[i][j+1] - v_d[i][j-1] )/(2*DY) )*DX2;
            }
        }
        ...
        double ** ud_ap = calloc(NX,sizeof(double *));	// Central coefficient of x-momentum dispersed phase
        double ** ud_an = calloc(NX,sizeof(double *));	// North coefficient of x-momentum dispersed phase
        double ** ud_as = calloc(NX,sizeof(double *));	// South coefficient of x-momentum dispersed phase
        double ** ud_ae = calloc(NX,sizeof(double *));	// East coefficient of x-momentum dispersed phase
        double ** ud_aw = calloc(NX,sizeof(double *));	// West coefficient of x-momentum dispersed phase
        for (i=0;i<NX;i++)
        {
        ...
        double ** vd_ap = calloc(NX,sizeof(double *));		// Central coefficient of y-momentum dispersed phase
        double ** vd_an = calloc(NX,sizeof(double *));		// North coefficient of y-momentum dispersed phase
        double ** vd_as = calloc(NX,sizeof(double *));		// South coefficient of y-momentum dispersed phase
        double ** vd_ae = calloc(NX,sizeof(double *));		// East coefficient of y-momentum dispersed phase
        double ** vd_aw = calloc(NX,sizeof(double *));		// West coefficient of y-momentum dispersed phase
        for (i=0;i<NX;i++)
        {
        ...
        for (j=1;j<NY;j++)
        {
            ...
            Qyd[m] = alphad_centre*( P[i][j-1] - P[i][j] )*DX + alphad_centre*RHO_D*g*DX2 + alphadn_centre*RHO_D*vn_d[i][j]*DX2*DT - 0.67*( alpha_d[i][j]*mu_total_D[i][j]*DIV_yd[i][j] - alpha_d[i][j-1]*mu_total_D[i][j-1]*DIV_yd[i][j-1] )*DX + F_added_y[i][j] + 0.5*fW*( v_d[i][j] - v_d[i-1][j] )*( z_w*psi_y_pos_x(v_d,i-1,j,1,1) - (1 - z_w)*psi_y_neg_x(v_d,i,j,1,1) ) + 0.5*fN*( v_d[i][j+1] - v_d[i][j] )*( (1 - z_n)*psi_y_neg_y(v_d,i,j+1,0,0) - z_n*psi_y_pos_y(v_d,i,j,0,0,v_inlet) ) + 0.5*fS*( v_d[i][j] - v_d[i][j-1] )*( z_s*psi_y_pos_y(v_d,i,j-1,0,0,v_inlet) - (1 - z_s)*psi_y_neg_y(v_d,i,j,0,0) );
                ...
                //// Effective Central and Neighboring Coefficients after application of PEA

        		// Lexicographic ordering of coefficients and variable to obtain 1D vector
                ...
                for (inner=1;inner<=ITERATION_X;inner++)
                {
                    ...
                    residual_x_c = GS_SOLVER_X(Qxcmod, Exc, Fxc, Dxc, Hxc, Bxc, Uc, u_c, resxc, nx, Kx);
        ...
        ///// Effective Coefficients for y-momentum after PEA
            ...
            for (inner=1;inner<=ITERATION_Y;inner++)
            {
                ...
                residual_y_c = GS_SOLVER_Y(Qycmod, Eyc, Fyc, Dyc, Hyc, Byc, Vc, v_c, resyc, NX, Ky);
        ...
        for (inner=1;inner<=ITERATION_X;inner++)
        {
            ...
            residual_x_d = GS_SOLVER_X(Qxdmod, Exd, Fxd, Dxd, Hxd, Bxd, Ud, u_d, resxd, nx, Kx);
        ...
        //// y-momentum solver for dispersed phase
            ...
            for (inner=1;inner<=ITERATION_Y;inner++)
            {
                ...
                residual_y_d = GS_SOLVER_Y(Qydmod, Eyd, Fyd, Dyd, Hyd, Byd, Vd, v_d, resyd, NX, Ky);
        ...
        //// Total Pressure-Correction equation 
        ...
        for (inner=1;inner<=ITERATION_P;inner++)
        {
            ...
            R2p = GS_SOLVER_Center(Qp, Ep, Fp, Dp, Hp, Bp, p, pp, resp, NX, Kp);
            ...
            if(inner <= 5)
            	R_scale = max(R2p,R_scale);

            residual_P = R2p/R_scale;
        ...
        // if(time == 250 || time == 500 || time == 1000)
        // {
        ...
        for (inner=1;inner<=ITERATION_ALPHA;inner++)
        {
            ...
            residual_alpha = GS_SOLVER_Center(Qa, Ea, Fa, Da, Ha, Ba, a, alpha_d, resa, NX, Kp);
        ...
        for (i=1;i<NX-1;i++)
        {
            for (j=1;j<NY-1;j++)
            {
            	// Volume fraction interpolation at the faces
                ...
                // Turbulent Viscosity interpolation at the faces
                ...
                // Switching functions for TVD scheme
        ...
        {
            R2k = GS_SOLVER_Center(Qk, Ek, Fk, Dk, Hk, Bk, K, k, resk, NX, Kp);
        ...
        for (inner=1;inner<=ITERATION_KEPS;inner++)
        {
            ...
            R2eps = GS_SOLVER_Center(Qe, Ee, Fe, De, He, Be, Eps, epsilon, reseps, NX, Kp);
    ...
    // Outer iteration loop end
        ...
        for (i=0;i<NX;i++)
        {
            for (j=0;j<NY;j++)
            {
                mut_C[i][j] = RHO_C*C_MU*pow(k[i][j],2)/epsilon[i][j] + 0.6*RHO_C*alpha_d[i][j]*DP*uslip_mag[i][j];  // Turbulent Viscosity calculation including BIT term
                mu_total_C[i][j] = mu + mut_C[i][j];
                mu_total_D[i][j] = mu_total_C[i][j]*RHO_D/RHO_C;	
            }
        }

        if ((residual_x_c<TOL_X_C) && (residual_y_c<TOL_Y_C) && (residual_x_d<TOL_X_D) && (residual_y_d<TOL_Y_D) && (residual_P<5e-2) && (residual_k<TOL_KEPS) && (residual_eps<TOL_KEPS) && (residual_alpha<TOL_ALPHA))
        {
...
if (time % SAVING_INTERVAL == 0)
{
 ...
 //// Exporting u-velocity
    ...
    sprintf(filename,"residualp%d.txt",time/SAVING_INTERVAL);
    alphac[count]=fopen(filename,"a");
    ...
    for (i=0;i<NX;i++)
    {
        for (j=0;j<NY;j++)
        {
            fprintf(alphac[count],"% 12.5e\t",residual[i][j]);
        }
        fprintf(alphac[count],"\n");
    }
    fclose(alphac[count]);
