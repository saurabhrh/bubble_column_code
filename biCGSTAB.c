    ...
    for (outer=1;outer<=OUTER;outer++)
    {
        ...
        for (j=1;j<NY;j++)
        {
            ...
            Qyd[m] = alphad_centre*( P[i][j-1] - P[i][j] )*DX + alphad_centre*RHO_D*g*DX2 + alphadn_centre*RHO_D*vn_d[i][j]*DX2*DT - 0.67*( alpha_d[i][j]*mu_total_D[i][j]*DIV_yd[i][j] - alpha_d[i][j-1]*mu_total_D[i][j-1]*DIV_yd[i][j-1] )*DX + F_added_y[i][j] + 0.5*fW*( v_d[i][j] - v_d[i-1][j] )*( z_w*psi_y_pos_x(v_d,i-1,j,1,1) - (1 - z_w)*psi_y_neg_x(v_d,i,j,1,1) ) + 0.5*fN*( v_d[i][j+1] - v_d[i][j] )*( (1 - z_n)*psi_y_neg_y(v_d,i,j+1,0,0) - z_n*psi_y_pos_y(v_d,i,j,0,0,v_inlet) ) + 0.5*fS*( v_d[i][j] - v_d[i][j-1] )*( z_s*psi_y_pos_y(v_d,i,j-1,0,0,v_inlet) - (1 - z_s)*psi_y_neg_y(v_d,i,j,0,0) );
                ...
                ILU0(Exc, Fxc, Dxc, Bxc, Hxc, fxc, dxc, bxc, exc, cxc, Kx, nx);

                residual_X(Qxcmod, Exc, Fxc, Hxc, Dxc, Bxc, rtilde_xc, Uc, u_c, nx);


                for (i=0;i<Kx;i++)
                {
                    G_xc[i] = rtilde_xc[i];
                    resxcold[i] = rtilde_xc[i];
                    // printf("%12.5e\n", fxc[i]);
                }
                ...
                for (inner=1;inner<=ITERATION_X;inner++)
                {
                    ...
                    residual_x_c = BiCGSTAB_Pre_x(G_xc, rtilde_xc, resxcold, s_xc, v_xc, t_xc, Exc, Fxc, Hxc, Dxc, Bxc, Uc, resxc, vec_Yxc_t, vec_Yxc_s, vec_deltaxc_t, vec_deltaxc_s, bxc, cxc, dxc, exc, fxc, Kx, nx, inner, u_c, Exc);
        ...
        ///// Stone's ILU Solver for y-momentum
            ...
            ILU0(Eyc, Fyc, Dyc, Byc, Hyc, fyc, dyc, byc, eyc, cyc, Ky, NX);

            residual_Y(Qycmod, Eyc, Fyc, Hyc, Dyc, Byc, rtilde_yc, Vc, v_c, NX);


            for (i=0;i<Ky;i++)
            {
                G_yc[i] = rtilde_yc[i];
                resycold[i] = rtilde_yc[i];
            }
            ...
            for (inner=1;inner<=ITERATION_Y;inner++)
            {
                ...
                residual_y_c = BiCGSTAB_Pre_y(G_yc, rtilde_yc, resycold, s_yc, v_yc, t_yc, Eyc, Fyc, Hyc, Dyc, Byc, Vc, resyc, vec_Yyc_t, vec_Yyc_s, vec_deltayc_t, vec_deltayc_s, byc, cyc, dyc, eyc, fyc, Ky, NX, inner, v_c, Eyc);
        ...
        ILU0(Exd, Fxd, Dxd, Bxd, Hxd, fxd, dxd, bxd, exd, cxd, Kx, nx);

        residual_X(Qxdmod, Exd, Fxd, Hxd, Dxd, Bxd, rtilde_xd, Ud, u_d, nx);


        for (i=0;i<Kx;i++)
        {
            G_xd[i] = rtilde_xd[i];
            resxdold[i] = rtilde_xd[i];
        }
        ...
        for (inner=1;inner<=ITERATION_X;inner++)
        {
            ...
            residual_x_d = BiCGSTAB_Pre_x(G_xd, rtilde_xd, resxdold, s_xd, v_xd, t_xd, Exd, Fxd, Hxd, Dxd, Bxd, Ud, resxd, vec_Yxd_t, vec_Yxd_s, vec_deltaxd_t, vec_deltaxd_s, bxd, cxd, dxd, exd, fxd, Kx, nx, inner, u_d, Exd);
        ...
        ILU0(Eyd, Fyd, Dyd, Byd, Hyd, fyd, dyd, byd, eyd, cyd, Ky, NX);

        residual_Y(Qydmod, Eyd, Fyd, Hyd, Dyd, Byd, rtilde_yd, Vd, v_d, NX);


        for (i=0;i<Ky;i++)
        {
            G_yd[i] = rtilde_yd[i];
            resydold[i] = rtilde_yd[i];
        }
            ...
            for (inner=1;inner<=ITERATION_Y;inner++)
            {
                ...
                residual_y_d = BiCGSTAB_Pre_y(G_yd, rtilde_yd, resydold, s_yd, v_yd, t_yd, Eyd, Fyd, Hyd, Dyd, Byd, Vd, resyd, vec_Yyd_t, vec_Yyd_s, vec_deltayd_t, vec_deltayd_s, byd, cyd, dyd, eyd, fyd, Ky, NX, inner, v_d, Eyd);
        ...
        //// Initializing Direction vector and Conjugate direction vector

        residual_center(Qp, Ep, Fp, Hp, Dp, Bp, rtilde_p, p, NX);


        for (i=0;i<Kp;i++)
        {
            G_p[i] = rtilde_p[i];
            respold[i] = rtilde_p[i];
        }


        // coefficient_matrix(Ep, Fp, Dp, Bp, Hp, fp, dp, bp, ep, cp, Kp, NX, ALPHA_P);

        ILU0(Ep, Fp, Dp, Bp, Hp, fp, dp, bp, ep, cp, Kp, NX);
        ...
        for (inner=1;inner<=ITERATION_P;inner++)
        {
            ...
            R2p = BiCGSTAB_Pre(G_p, rtilde_p, respold, s_p, v_p, t_p, Ep, Fp, Hp, Dp, Bp, p, resp, vec_Yp_t, vec_Yp_s, vec_deltap_t, vec_deltap_s, bp, cp, dp, ep, fp, Kp, NX, pp);
        ...
        residual_center(Qa, Ea, Fa, Ha, Da, Ba, rtilde_a, a, NX);
        ...
        for (inner=1;inner<=ITERATION_ALPHA;inner++)
        {
            ...
            residual_alpha = BiCGSTAB_Pre(G_a, rtilde_a, resaold, s_a, v_a, t_a, Ea, Fa, Ha, Da, Ba, a, resa, vec_Ya_t, vec_Ya_s, vec_deltaa_t, vec_deltaa_s, ba, ca, da, ea, fa, Kp, NX, alpha_d);
        ...
        ILU0(Ek, Fk, Dk, Bk, Hk, fk, dk, bk, ek, ck, Kp, NX);

        residual_center(Qk, Ek, Fk, Hk, Dk, Bk, rtilde_k, K, NX);
        ...
        {
            R2k = BiCGSTAB_Pre(G_k, rtilde_k, reskold, s_k, v_k, t_k, Ek, Fk, Hk, Dk, Bk, K, resk, vec_Yk_t, vec_Yk_s, vec_deltak_t, vec_deltak_s, bk, ck, dk, ek, fk, Kp, NX, k);
        ...
        ILU0(Ee, Fe, De, Be, He, fe, de, be, ee, ce, Kp, NX);

        residual_center(Qe, Ee, Fe, He, De, Be, rtilde_eps, Eps, NX);

        //// Initializing Direction vector and Conjugate direction vector

        for (i=0;i<Kp;i++)
        {
            G_eps[i] = rtilde_eps[i];
            resepsold[i] = rtilde_eps[i];
        }
        ...
        for (inner=1;inner<=ITERATION_KEPS;inner++)
        {
            ...
            R2eps = BiCGSTAB_Pre(G_eps, rtilde_eps, resepsold, s_eps, v_eps, t_eps, Ee, Fe, He, De, Be, Eps, reseps, vec_Yeps_t, vec_Yeps_s, vec_deltaeps_t, vec_deltaeps_s, be, ce, de, ee, fe, Kp, NX, epsilon);
